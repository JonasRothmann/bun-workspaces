...
var packages = (await Promise.all(workspaces.map((workspace) => "./" + workspace.replace("/*", "")).map(async (workspace) => {
  const dir = await readdir(workspace, { withFileTypes: true });
  const dirs = await Promise.all(dir.map(async (dir2) => {
    if (dir2.isDirectory()) {
      try {
        const rootPath = `${workspace}/${dir2.name}`;
        const path2 = `${workspace}/${dir2.name}/package.json`;
...
program2.option("--filter <type>", "a filter option").arguments("<cmd> [args...]").action(async (cmd, args, options) => {
  ...
  if (filterName) {
    ...
    packageJson.dependencies = Object.fromEntries(Object.entries(dependencies).filter(([key, value]) => {
    ...
    await Bun.write(filter.path + "/package.json", JSON.stringify(packageJson, null, 2));
  }
